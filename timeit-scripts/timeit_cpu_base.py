...
if __name__ == "__main__":
    ...
    SETUP_CODE = \
        ...
        f"from newton_eval import eval_driver_numba_cpu\n" \
        ...
        f"yy_numba_gpu = eval_driver_numba_cpu(xx_test[:5], nwt_coeffs, exponents, gen_points)\n" \

    n_reps = 1
    n_loops = 2
    t = timeit.Timer(f"eval_driver_numba_cpu(xx_test, nwt_coeffs, exponents, gen_points)", setup=SETUP_CODE).repeat(n_reps, n_loops)
    ...
    np.savetxt(f"/p/home/jusers/wicaksono1/jureca/cpu-base-timing-{timestamp}.txt", t_summary)
