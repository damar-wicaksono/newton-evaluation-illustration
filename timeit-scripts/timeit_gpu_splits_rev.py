...
if __name__ == "__main__":
    ...
    SETUP_CODE = \
        ...
        f"from newton_eval import eval_driver_numba_gpu_splits_rev\n" \
        ...
        f"yy_numba_gpu = eval_driver_numba_gpu_splits_rev(xx_test[:5], nwt_coeffs, exponents, gen_points, splits={splits})\n" \

    n_reps = 5
    n_loops = 2
    t = timeit.Timer(f"eval_driver_numba_gpu_splits_rev(xx_test, nwt_coeffs, exponents, gen_points, splits={splits})", setup=SETUP_CODE).repeat(n_reps, n_loops)
    ...
    np.savetxt(f"/p/home/jusers/wicaksono1/jureca/results/gpu-timing-degree-{poly_degree}-splits-rev-{splits}-{timestamp}.txt", t_summary)
