...
__all__ = ["eval_driver_numba_gpu_splits_rev"]
...
def eval_newton_polynomial(
    ...
    starts_ends: np.ndarray,
    idx: int,
) -> None:
    """Evaluate the Newton polynomial multiple query points (CUDA kernel)."""
    row = cuda.grid(1)
    N = exponents.shape[0]
    start_0 = starts_ends[idx][0]
    end_0 = starts_ends[idx][1]
...
def eval_driver_numba_gpu_splits_rev(
    xx: np.ndarray,
    ...
    # Start ends array
    starts_ends = np.empty((splits, 2), dtype=np.int32)
    for i in range(splits):
        ...
        starts_ends[i][0] = start
        starts_ends[i][1] = end
    starts_ends_global_mem = cuda.to_device(starts_ends)
